# 文章_Linux运维_Bash脚本_编译安装FFmpeg-3.4.13_GF_2023-03-13
...
# * CMake >= 3.14.0
# * Pango == 1.51.x (Contains: Fribidi, FreeType)
...
# ---------------- FFmpeg - 3.4.13 -----------------
# Need File: ffmpeg-3.4.13.tar.xz
...
# Function: 编译安装(Compile Install) FFmpeg-3.4.13
# ##################################################
function Compile_Install_FFmpeg_3_4_13() {

    if [[ ! -d "/opt/ffmpeg-3.4.13" ]]; then
        ...
        # ------------------------------------------
        read -p "[Confirm] Compile and Install ( ffmpeg-3.4.13 )? (y/n)>" VERIFY
        if [[ "$VERIFY" != "y" ]]; then exit 1; fi

        # ------------------------------------------
        tar -xvJf $STORAGE/ffmpeg-3.4.13.tar.xz && STEP_UNZIPPED=1
        
        # ------------------------------------------
        if [[ $STEP_UNZIPPED == 1 ]]; then
            sed -i "289s/encoderDelay/nDelay/" $STORAGE/ffmpeg-3.4.13/libavcodec/libfdk-aacenc.c
        fi
        ...
        #               --enable-libx265 \
        #               --enable-openssl \
        #               --docdir=/usr/share/doc/ffmpeg-3.4.13
        # ..........................................
        ...
        # ..........................................
        # * Problem: ERROR: libass not found using pkg-config
        ...
        cd $STORAGE/ffmpeg-3.4.13 && ./configure --prefix=/opt/ffmpeg-3.4.13 \
                                                 --enable-gpl \
                                                 --enable-version3 \
                                                 --enable-nonfree \
                                                 --disable-static \
        ...
        if [[ $STEP_INSTALLED == 1 ]]; then
            ...
            if [[ ! -d "/opt/lib/pkgconfig" ]]; then mkdir /opt/lib/pkgconfig; fi
            # ......................................
            ln -sf /opt/ffmpeg-3.4.13/bin/ffmpeg  /usr/local/bin/
            ln -sf /opt/ffmpeg-3.4.13/bin/ffprobe /usr/local/bin/
            # ......................................
            rsync -av /opt/ffmpeg-3.4.13/include/ /usr/local/include/
            rsync -av /opt/ffmpeg-3.4.13/lib/     /usr/local/lib/
            # ......................................
            cp -f /opt/ffmpeg-3.4.13/lib/pkgconfig/*.pc /opt/lib/pkgconfig/
        fi

        # ------------------------------------------
        cd $STORAGE && rm -rf $STORAGE/ffmpeg-3.4.13 && return 0
    else
    
        echo "[Caution] Path: ( /opt/ffmpeg-3.4.13 ) Already Exists."
        # ------------------------------------------
        return 0
    fi
}
...
以上就是关于 Linux运维 Bash脚本 编译安装FFmpeg-3.4.13 的全部内容。
