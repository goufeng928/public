# 文章_Shell教程_不同Shell中环境变量的设置和读取方式的差异_GF_2023-08-20
...
* 读取环境变量: 在 Fish 中, 环境变量自动被设置为全局变量, 无需特定语法, 使用 $ 符号直接引用环境变量。
...
以上就是关于 Shell教程 不同Shell中环境变量的设置和读取方式的差异 的全部内容。
